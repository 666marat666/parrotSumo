...
Sumo.prototype.forward = function(val) {
  if (this._pcmd) {
    this._pcmd.flag = 1;
    this._pcmd.speed = validate_val(val);
  }
...
Sumo.prototype.backward = function(val) {
  if (this._pcmd) {
    this._pcmd.flag = 1;
    this._pcmd.speed = validate_val(val) * -1;
  }
...
Sumo.prototype.right = function (val) {
  if (this._pcmd) {
    this._pcmd.flag = 1;
    this._pcmd.turn = validate_val(val);
  }
  return this; 
}
...
Sumo.prototype.left = function (val) {
  if (this._pcmd) {
    this._pcmd.flag = 1;
    this._pcmd.turn = validate_val(val) * -1;
  }
...
Sumo.prototype.move = function(speed, turn) {
  //Value negative to go left, positive = right
  this._pcmd = {
    flag: 1,
    speed: speed >= 0 ? validate_val(speed) : validate_val(speed) * -1,
    turn: turn >= 0 ? validate_val(turn) : validate_val(turn) * -1
  };

  return this;
}

Sumo.prototype.angle = function (angle) {
  console.log(((Math.PI * angle) / 180));
  this._generateCapOffset(((Math.PI * angle) / 180));
}

function validate_val(val) {
val = val < 0 ? val = (val * -1) : val;
  if (val > 100) {
    return 100;
  } else if (val < 0) {
    return 0;
...
Sumo.prototype._generateCapOffset = function(angle) {
  ...
  buf.writeUInt8(constants.ARCOMMANDS_ID_PROJECT_JUMPINGSUMO, 0);
  buf.writeUInt8(constants.ARCOMMANDS_ID_JUMPINGSUMO_CLASS_PILOTING, 1);
  buf.writeUInt16LE(constants.ARCOMMANDS_ID_JUMPINGSUMO_PILOTING_CMD_ADDCAPOFFSET, 2);
  buf.writeFloatLE(angle, 4);

  return this._writePacket(this._networkFrameGenerator(buf));
};

Sumo.prototype.setPositionForRun = function(latitude, longitude) {
  var buf = new Buffer(6);

  buf.writeUInt8(constants.ARCOMMANDS_ID_PROJECT_COMMON, 0);
  buf.writeUInt8(constants.ARCOMMANDS_ID_COMMON_CLASS_GPS, 1);
  buf.writeUInt16LE(constants.ARCOMMANDS_ID_COMMON_GPS_CMD_CONTROLLERPOSITIONFORRUN, 2);
  buf.writeDoubleLE(latitude, 4);
